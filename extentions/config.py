...
class Config:
    def __init__(self):
        ...
        # ABILITY SWITCH
        self.test = os.environ.get("TEST_MODE", "False").lower() in ("true", "1", "yes")  # TESTMODE(Switch to OverRein), default = False
        ...
        self.voice_tokens = [
            os.environ["DISCORD_VOICE_TOKEN_01"],
            os.environ["DISCORD_VOICE_TOKEN_02"],
            os.environ["DISCORD_VOICE_TOKEN_03"],
            os.environ["DISCORD_VOICE_TOKEN_04"],
        ...
        self.remind = (
            1140326740158333048 if self.test is False else self.remind_TEST
        )  # リマインド
        self.ake_news_test = 1166921222785859684  # メンテナンス部屋/ニューステスト
        self.ake_news = (
            1166620310750113802 if self.test is False else self.ake_news_test
        )  # ニュース

        self.cathedral = (
            1183254115338420285 if self.test is False else self.ake_news_test
        )  # 聖堂

        self.maintenance = 1081251314958344313  # メンテナンス
        self.request = (
            1093849433621401600 if self.test is False else 1285907503695007846
        )  # サポートリクエスト
        self.request_url = (
            f"https://discord.com/channels/{self.main_server}/{self.request}"
            if self.test is False
            else f"https://discord.com/channels/{self.testserverid}/{self.request}"
        )
        ...
        self.screenshot_recruit_channel = (
            1284115621465948171  # 公開求人ツール（スクショ認識）
        )
        ...
        self.multiplay_request_channel = 1306450120669139024  #マルチ募集
    ...
    def load_dynamic_config(self):
        with open(
            os.path.join(self.dir, "configs\\dynamic.json"), "r", encoding="utf-8"
        ) as f:
            data = json.load(f)
        self.dynamic = data
    ...
    def write_dynamic_config(self):
        with open(
            os.path.join(self.dir, "configs\\dynamic.json"), "w", encoding="utf-8"
        ) as f:
